...
class HotVsColdObservablesScala {
  val log = LoggerFactory.getLogger(classOf[HotVsColdUtilities])
...
object HotVsColdObservablesScala extends App{
  ...
  val DEFAULT_COLD_OBSERVABLE = false
  val mapOfDisposable = Map.empty[Integer, Disposable]
...
//      observer.onError(new RuntimeException("Simulating an error that will halt the entire stream of data. Data=" + prime))
  ...
  def createObservable(isHotObservable: Boolean, threshold: Int): Observable[Future[Either[String,Int]]] = {
...
*/
  ...
  def runObservable(isHotObservable: Boolean): Unit = {
    ...
    var observable =
      Observable.create[Int](observer => {
        nextPrime(START_PRIME_AT_1, DEFAULT_THRESHOLD, observer)
        Subscription()
      })
        .switchMap[Future[Either[String,Int]]](prime => {
